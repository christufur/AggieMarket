...
function requireSeedUser(index: number): SeedUser {
    const user = insertedUsers[index];
    if (!user) {
        throw new Error(`Expected seeded user at index ${index}`);
    }
    return user;
}

const _admin = requireSeedUser(0);
const alex = requireSeedUser(1);
const mia = requireSeedUser(2);
const carlos = requireSeedUser(3);
const priya = requireSeedUser(4);
const jay = requireSeedUser(5);
